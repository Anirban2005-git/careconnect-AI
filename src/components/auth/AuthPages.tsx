...
export function AuthPages() {
  ...
  const handleDemoLogin = async () => {
    ...
    setError("");
    try {
      await login("demo@careconnect.in");
    } catch (err: any) {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 text-center text-xs text-slate-500 space-y-2">
              {view === "login" && (
                <>
                  <button onClick={() => setView("signup")} className="text-teal-700 font-semibold hover:underline">
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-[11px] text-slate-500">
            ...
            <span>256-bit encrypted • ABHA & NDHM ready • Demo mode available</span>
          </div>
        </div>
      </div>
    </div>
